...
object MapChestManager {
    ...
    fun generateChest(uuid: UUID, world: World, loc: Location): BlockPos {
        ...
        OnTheQuestBukkit.plugin.logger.info("Generated new map $uuid with seed $seed (${world.seed} ${loc.blockX} ${loc.blockY} ${loc.blockZ} ${loc.yaw.toLong()} $salt)")
    ...
    fun init() {
        if (!OnTheQuestBukkit.plugin.dataFolder.exists())
            OnTheQuestBukkit.plugin.dataFolder.mkdirs()

        val file = File(OnTheQuestBukkit.plugin.dataFolder, "chest_generation.json")
        if (file.exists()) {
    ...
    fun checkGenerationQueue(chunk: Chunk) {
        OnTheQuestBukkit.plugin.server.scheduler.runTaskAsynchronously(OnTheQuestBukkit.plugin, Runnable {
            ...
            chests.forEach {
                ...
                OnTheQuestBukkit.plugin.logger.info("Generated chest ${it.uuid} at ${mutablePos.x} ${mutablePos.y} ${mutablePos.z}")
            ...
            OnTheQuestBukkit.plugin.server.scheduler.runTask(OnTheQuestBukkit.plugin, Runnable {
                chestPlacementQueue.forEach {
                    ...
                    OnTheQuestBukkit.plugin.logger.info("Placed chest ${it.uuid} at ${it.pos.x} ${it.pos.y} ${it.pos.z}")
                }
            })
